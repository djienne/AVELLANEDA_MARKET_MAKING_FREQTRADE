...
def colorize_volume(volume: Optional[float]) -> str:
    if volume is None:
        return f"{Colors.DIM}-{Colors.RESET}"
    # Format large numbers with appropriate units
    if volume >= 1_000_000:
        return f"{Colors.WHITE}{volume/1_000_000:.1f}M{Colors.RESET}"
    elif volume >= 1_000:
        return f"{Colors.WHITE}{volume/1_000:.1f}K{Colors.RESET}"
    else:
        return f"{Colors.WHITE}{volume:.2f}{Colors.RESET}"
...
def main() -> None:
    ...
    for c in conts:
        ...
        # Extract volume data - use the correct field name from Freqtrade API
        volume = None
        if isinstance(prof, dict):
            volume = prof.get("trading_volume")
    ...
    print(
        f"{Colors.DIM}Rules - Profit: >0 good; Win Rate: >=50% good; PF: >=1.0 good; Max DD: >=-20% good; CAGR: >10% good.{Colors.RESET}\n"
    )
